...
public class AdminPersonas {
	...
	public static void insertar(Persona persona) throws Exception {
		...
		/*
		 * try { con= ConexionBDD.conectar(); //AQUI LLAMO EL METODO CONECTAR CADA QUE
		 * VALLA NECESITAR System.out.println("Insertar"); }catch (Exception e) {
		 * //mostrar el error al uusuario System.out.println(e.getMessage()); //logger
		 * el error e.printStackTrace(); }finally { //cerrar la coneccion //con este
		 * finally cerramos la coneccion try { con.close(); }catch(SQLException e) {
		 * e.printStackTrace(); System.out.println("Error de infraestructura"); } }
		 */
	}
	...
	public static void actualizar(Persona persona) throws Exception {
		...
		try {
			...
			ps = con.prepareStatement("UPDATE personas SET " + "nombre = ?, " + // 1
					"apellido = ?, " + // 2
					"estatura = ?, " + // 3
					"fecha_nacimiento = ?, " + // 4
					"hora_nacimiento = ?, " + // 5
	...
	public static ArrayList<Persona> buscarPorNombre(String nombreBusqueda) throws Exception {
		...
		Connection con=null;
		PreparedStatement ps= null;
		ResultSet rs=null; //contiene filas y columnas
		try {
			con=ConexionBDD.conectar();
			...
			LOGGER.error("Erro al consultar nombre", e);
			throw new Exception("Erro al consultar nombre  ");
	...
	public static ArrayList<Persona> buscarPorCedula(String cedulaBusqueda) throws Exception {
		ArrayList<Persona> persona = new ArrayList<Persona>();
		Connection con=null;
		PreparedStatement ps= null;
		ResultSet rs=null; //contiene filas y columnas
		try {
			...
			LOGGER.error("Erro al consultar la cedula", e);
			throw new Exception("Erro al consultar cedula ");
	...
	public static ArrayList<Persona> buscarApellido(String apellidoBuscar) throws Exception {
		ArrayList<Persona> persona = new ArrayList<Persona>();
		Connection con=null;
		PreparedStatement ps= null;
		ResultSet rs=null; //contiene filas y columnas
		try {
			...
			LOGGER.error("Erro al consultar el apellido", e);
			throw new Exception("Erro al consultar el apellido");
